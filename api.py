...
<<<<<<< HEAD
from tools import logical_model_core, physical_model_core
=======
>>>>>>> b0580f33ccf951d74a973fa197297778c190b61c
import logging
...
@app.post("/generate-conceptual")
def generate_conceptual(payload: RequirementRequest, request: Request):

    result = DataModelingOrchestrator().run(payload.requirement)
    conceptual_output = result.get("conceptual_output")

    if not conceptual_output:
        return {"error": "No conceptual model generated"}

    conceptual = ConceptualModel.model_validate(conceptual_output)
    conceptual = _apply_generated_mermaid(conceptual)

    artifact_id = save_conceptual_artifact(conceptual)
    links = _build_artifact_links(request, "conceptual", artifact_id)

    return {
        "conceptual_output": conceptual.model_dump(),
...
@app.post("/update-conceptual")
def update_conceptual(payload: UpdateRequest, request: Request):

    model = payload.existing_model
    relationships = model.get("relationships", [])

    # apply changes here...

    conceptual = ConceptualModel.model_validate(model)
    conceptual = conceptual.model_copy(update={"relationships": relationships})
    conceptual = _apply_generated_mermaid(conceptual)
...
@app.post("/generate-logical")
def generate_logical(payload: dict):

    conceptual = payload["conceptual_model"]

    logical = logical_model_core(conceptual)
    logical = LogicalModel.model_validate(logical)
    logical = _apply_generated_logical_mermaid(logical)
    artifact_id = save_logical_artifact(logical)
    links = _build_artifact_links(request, "logical", artifact_id)

    return {
        "logical_output": logical.model_dump(),
        "logical_view_url" : links["view_url"],
        "logical_download_json_url": links["download_json_url"],
        "logical_download_mermaid_url": links["download_mermaid_url"]
    }

@app.post("/generate-physical")
def generate_physical(payload: dict):

    logical = payload["logical_model"]

    physical = physical_model_core(logical)
    physical = PhysicalModel.model_validate(physical)
    physical = _apply_generated_physical_mermaid(physical)
    artifact_id = save_logical_artifact(physical)
    links = _build_artifact_links(request, "physical", artifact_id)

    return {
        "physical_output": physical.model_dump(),
        "physical_view_url" : links["view_url"],
        "physical_download_json_url": links["download_json_url"],
        "physical_download_mermaid_url": links["download_mermaid_url"]
